...
export function BenefitProgress({
  ...
  const percentage =
    benefit.totalValue > 0
      ? Math.min(100, (benefit.usedValue / benefit.totalValue) * 100)
      : 0;

  return (
    <article
      className={cn(
        "panel panel-body flex h-full min-w-0 flex-col",
        ["expired", "inactive"].includes(benefit.status) && "bg-transparent",
      )}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <h3 className="break-words text-base font-medium">{benefit.name}</h3>
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[11px] text-muted-foreground">
              {["expired", "inactive"].includes(benefit.status)
                ? "unused · unavailable"
                : "remaining"}
            </p>
          </div>
          <p className="text-right text-xs tabular-nums text-muted-foreground">
            {formatCurrency(benefit.usedValue)} of{" "}
            {formatCurrency(benefit.totalValue)} used
          </p>
        </div>
        <Progress
          value={percentage}
          aria-label={`${benefit.name} value used`}
          className={cn(
            "mt-4",
            benefit.status === "expiring" &&
              "[&_[data-slot=progress-indicator]]:bg-primary",
          )}
        />
        <p className="mt-3 text-[11px] leading-5 text-muted-foreground">
          {FREQUENCY_LABELS[benefit.resetFrequency] ?? benefit.resetFrequency}
          {benefit.effectiveExpiry
            ? ` / ends ${formatDate(benefit.effectiveExpiry)}`
            : ""}
        </p>
      </div>
